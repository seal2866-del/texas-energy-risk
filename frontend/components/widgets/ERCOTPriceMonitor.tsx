...
import { TrendingUp, TrendingDown, Clock, WifiOff, AlertTriangle, ShieldAlert } from "lucide-react";
import type { ERCOTPrice, ErcotVerification } from "@/lib/api";
import { formatPrice } from "@/lib/utils";

interface Props {
  prices:             ERCOTPrice[];
  loading?:           boolean;
  ercotVerification?: ErcotVerification;
}
...
export default function ERCOTPriceMonitor({ prices, loading, ercotVerification }: Props) {
  ...
  const verifIsValid  = ercotVerification?.is_valid ?? (current > 0);
  const verifStatus   = ercotVerification?.status ?? "real-time";
  const verifReason   = ercotVerification?.reason ?? null;
  const lastKnownPrice = ercotVerification?.last_known_price ?? null;
  const lastKnownTs    = ercotVerification?.last_known_ts ?? null;
  const isPending      = verifStatus === "pending_confirmation";

  // Display price: if invalid, fall back to last-known-good
  const displayPrice   = verifIsValid ? current : (lastKnownPrice ?? null);
  const isLastKnown    = !verifIsValid && displayPrice !== null;
  const outsideNormal  = (displayPrice ?? 0) >= PRICE_HIGH_WARNING;
  ...
  // Chart: skip invalid zero/null values — show gaps instead of misleading zeros
  const chartData = prices.slice(-24)
    .filter(p => p.price_mwh > 0)
    .map((p) => ({
  ...
  return (
    <div className={`card-glass p-6 border transition-all ${outsideNormal ? "border-red-500/20" : "border-white/5"}`}>
      ...
      <div className="flex items-start justify-between mb-1">
        ...
        {!loading && cacheSize > 0 && (
          isPending ? (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border bg-blue-500/10 border-blue-500/25 text-blue-400">
      ...
      {/* Verification reason / last-known notice */}
      {!loading && (isPending || isLastKnown) && verifReason && (
        <div className={`mb-3 px-3 py-2 rounded-lg text-xs border ${
          ...
          {isPending ? (
            <span className="font-semibold">Large price movement detected — </span>
          ) : (
            <span className="font-semibold">Showing last known valid price. </span>
          )}
          {verifReason}
        </div>
      )}
      ...
      <div className="flex items-end justify-between mb-4">
        <div className="flex flex-col gap-1">
          {!loading && cacheSize < 2 && !isLastKnown && (
          ...
          {outsideNormal && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-500/15 border border-red-500/25 text-red-400 text-xs font-semibold">
              <AlertTriangle className="w-3 h-3" />Outside normal range
            </span>
          )}
        </div>
        ...
        <div className="text-right">
          {loading ? (
            ...
            <div>
              ...
              <p className="text-xs text-gray-600 mt-0.5">Data temporarily unavailable</p>
            </div>
          ) : (
            <>
              <p className={`text-2xl font-black ${outsideNormal ? "text-red-400" : isLastKnown ? "text-amber-300" : "text-white"}`}>
                {formatPrice(displayPrice)}<span className="text-sm font-normal text-gray-400">/MWh</span>
              </p>
              {isLastKnown && lastKnownTs && (
                <p className="text-xs text-amber-400/70 mt-0.5">Last known valid price</p>
              )}
              {!isLastKnown && reliable && (
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="price" stroke={strokeColor} strokeWidth={2} fill={`url(#${gradientId})`} dot={false} connectNulls={false} />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>
