      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest">
            Texas Weather Demand Risk
          </p>
          {tomorrow && (
            <p className="text-xs text-gray-600 mt-0.5">{tomorrow.location_name}</p>
          )}
        </div>
        ...
        {forecasts.slice(0, 5).map((f, i) => {
          ...
          return (
            <div key={i} className="text-center py-2 rounded-lg bg-white/3">
              ...
              <p className={cn("text-xs font-semibold", riskColor(f.demand_risk))}>
                {f.temp_high_f.toFixed(0)}°
              </p>
            </div>
          );
        })}
