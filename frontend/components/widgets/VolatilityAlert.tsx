...
export default function VolatilityAlert({ signal }: Props) {
  ...
  const isOutsideNormal = !triggered && value !== null && value >= 105;

  return (
    <div className={cn("card-glass p-5 border transition-all", triggered ? riskBg(severity) : "border-white/5")}>
      <div className="flex items-start gap-3">
        <div className={cn(
          "mt-0.5 flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center",
          triggered ? (severity === "high" ? "bg-red-500/20" : "bg-amber-500/20") : "bg-white/5"
        )}>
          {triggered
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <p className="text-sm font-semibold text-white">{title || "ERCOT Volatility Monitor"}</p>
            <div className="flex items-center gap-1.5">
              {triggered && (
                <span className={cn(
                  ...
                  <Zap className="w-3 h-3" />Alert Triggered
                </span>
              )}
              <span className={cn(
          ...
          {isOutsideNormal && (
            <p className="mt-1.5 text-xs text-amber-400 font-medium">Outside normal range</p>
          )}

          {value !== null && triggered && (
            <div className="mt-2 text-xs text-gray-500">
              Current: <span className={cn("font-semibold", riskColor(severity))}>{formatPrice(value)}/MWh</span>
          ...
          {triggered && confidence !== null && confidence !== undefined && (
            <p className="mt-2 text-xs text-gray-600">
              Signal confidence:{" "}
              <span className={cn("font-semibold", confidence >= 75 ? "text-green-400" : confidence >= 55 ? "text-amber-400" : "text-gray-500")}>
                {confidence}%
              </span>
            </p>
          )}
          ...
          {time_horizon && (
            <p className="mt-2 text-xs text-gray-600 font-mono">{time_horizon}</p>
          )}
        </div>
      </div>
    </div>
