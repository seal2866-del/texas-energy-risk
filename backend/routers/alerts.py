...
async def send_test_sms(authorization: str = Header(default="")):
    ...
    if not sms_phone:
        raise HTTPException(status_code=400, detail="No SMS phone number configured in alert preferences")

    from services.alert_service import _send_sms
    body = "TX Energy Risk TEST: SMS alerts are active. You will receive risk change notifications at this number. texasgridintel.com"
