...
async def get_latest_approved():
    ...
    sb = get_supabase()
    r  = sb.table("newsletter_issues")            .select("id, subject, html_content, created_at")            .eq("status", "approved")            .order("created_at", desc=True)            .limit(1)            .execute()
    if not r.data:
        raise HTTPException(status_code=404, detail="No approved newsletter issue found")
    issue = r.data[0]
    html = issue.get("html_content") or ""
    if not html:
        raise HTTPException(status_code=404, detail="Latest approved issue has no HTML content")
    return {
        "id":        issue["id"],
        "subject":   issue.get("subject") or "Texas Grid Intel Newsletter",
        "html_body": html,
    }
